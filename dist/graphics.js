  ...
  return {
    ...
    execute: function () {
      Sunburst = function Sunburst() {
        ...
        var y = d3.scale.sqrt().range([0, this.radius]);

        var color = d3.scale.category20c();
        this.init = function () {
          d3.select(self.panelId).remove();

          var svg = d3.select(self.selector).append(self.panelId).append("svg").attr("width", self.width).attr("height", self.height).append("g").attr("transform", "translate(" + self.width / 2 + "," + (self.height / 2 + 10) + ")");
          ...
          d3.json("public/plugins/grafana-sunburst-panel/flare.json", function (error, root) {
            ...
            node = root;
            var path = svg.datum(root).selectAll("path").data(partition.nodes).enter().append("path").attr("d", arc).style("fill", function (d) {
